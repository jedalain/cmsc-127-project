...
export const addFoodEstablishment = async (
  ...
  try {
    ...
    res
      .status(201)
      .json(convertBigInt({ id: result.insertId, name, address, userId }));
  } catch (error) {
    console.error(error);
...
export const updateFoodEstablishment = async (req: Request, res: Response) => {
  try {
    ...
    const sql =
      "UPDATE foodEstablishments SET name = ?, address = ? WHERE establishmentId = ?";
...
export const deleteFoodEstablishment = async (req: Request, res: Response) => {
  try {
    ...
    //delete estab
    const deleteEstablishmentSql =
      "DELETE FROM foodEstablishments WHERE establishmentId = ?";
